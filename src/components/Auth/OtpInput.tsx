      ...
      {Array.from({ length }).map((_, index) => {
        ...
        return (
          <input
            ...
            onPaste={handlePaste}
            disabled={disabled}
            className={`w-11 h-13 sm:w-12 sm:h-14 text-center text-xl font-mono font-bold rounded-xl border transition-all duration-150 outline-none shadow-sm caret-cyan-400 [color-scheme:dark]
              ${
                hasError
                  ? 'border-red-500 bg-red-500/10 text-red-200 focus:bg-red-500/20 focus:text-red-100 focus:ring-2 focus:ring-red-400/30'
                  : isFilled
                  ? 'border-cyan-500/60 bg-slate-900/80 text-cyan-300 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-500/30 focus:bg-slate-900'
                  : 'border-slate-800 bg-slate-900/60 text-slate-100 placeholder:text-slate-500/70 hover:border-slate-700 focus:border-cyan-500/60 focus:ring-2 focus:ring-cyan-500/30 focus:bg-slate-900'
              }
